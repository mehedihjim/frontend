...
export const ExpenseTable = ({
  ...
  const currentItems = expenseData?.data?.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );

  // Determine the total number of pages
  const totalPages = Math.ceil(expenseData?.data?.length / itemsPerPage);
  if (isLoading) return <p>Loading...</p>;
  return (
    <>
      <div className="mb-6 mt-6 ">
        <div className="table-headline mb-6 dark:text-white">Expense List</div>
      </div>
